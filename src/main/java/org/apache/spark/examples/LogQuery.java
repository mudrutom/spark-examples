...
public final class LogQuery {
	...
	public static Tuple3<String, String, String> extractKey(String line) {
		final Matcher m = apacheLogRegex.matcher(line);
		if (m.find()) {
			final String ip = m.group(1), user = m.group(3), query = m.group(5);
	...
	public static void main(String[] args) {
		final SparkConf sparkConf = new SparkConf().setAppName("LogQuery");
		final JavaSparkContext jsc = new JavaSparkContext(sparkConf);

		final JavaRDD<String> dataSet = (args.length == 1) ? jsc.textFile(args[0]) : jsc.parallelize(exampleApacheLogs);
		final JavaPairRDD<Tuple3<String, String, String>, Stats> extracted = dataSet.mapToPair(s -> new Tuple2<>(extractKey(s), extractStats(s)));
		final JavaPairRDD<Tuple3<String, String, String>, Stats> counts = extracted.reduceByKey((stats, stats2) -> stats.merge(stats2));

		final List<Tuple2<Tuple3<String, String, String>, Stats>> output = counts.collect();
		for (Tuple2 t : output) {
