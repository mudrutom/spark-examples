...
public final class NetworkWordCount {
	...
	public static void main(String[] args) {
		...
		final JavaReceiverInputDStream<String> lines = ssc.socketTextStream(hostname, port, StorageLevels.MEMORY_AND_DISK_SER);
		final JavaDStream<String> words = lines.flatMap(x -> Lists.newArrayList(SPACE.split(x)));
		final JavaPairDStream<String, Integer> wordCounts = words.mapToPair(s -> new Tuple2<>(s, 1)).reduceByKey((i1, i2) -> i1 + i2);
