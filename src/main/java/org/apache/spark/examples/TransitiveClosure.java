 ...
 * Usage: TransitiveClosure [slices]
 */
public final class TransitiveClosure {
	...
	static List<Tuple2<Integer, Integer>> generateGraph() {
		final Set<Tuple2<Integer, Integer>> edges = new HashSet<>(numEdges);
	...
	public static void main(String[] args) {
		final SparkConf sparkConf = new SparkConf().setAppName("HdfsLR");
		final JavaSparkContext sc = new JavaSparkContext(sparkConf);
		final Integer slices = (args.length > 0) ? Integer.parseInt(args[0]) : 2;
		...
		final JavaPairRDD<Integer, Integer> edges = tc.mapToPair(e -> new Tuple2<>(e._2(), e._1()));
